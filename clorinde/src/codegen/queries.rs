...
use crate::{
    codegen::{ModCtx, types::is_nullable_field_hack},
    config::Config,
    ...
    let impl_tokens = if let Some((idx, index)) = row {
        ...
        if *is_named {
            ...
            let field_type = syn::parse_str::<syn::Type>({
                ...
                field.is_nullable = is_nullable_field_hack(&field.ty);
        ...
        if param.is_named {
            ...
            if let Some((idx, _)) = row {
                ...
                let query_row_struct = if prepared_row.is_named {
                    ...
                    syn::parse_str::<syn::Type>({
                        let mut prepared_row = prepared_row.fields[0].clone();
                        prepared_row.is_nullable = is_nullable_field_hack(&prepared_row.ty);
                        &prepared_row.own_struct(ctx)
                    })
                    .unwrap()
                };
